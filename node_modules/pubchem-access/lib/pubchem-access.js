(function(root, factory) {
    ...
    Find.prototype.exec = function (url) {
        function execute (callback, dataFormat, optionF) {
            ...
            this.substructure = true
            if (this.substructure) {
                url = url.replace(/compound\//, "compound/substructure/")
                url = url.replace(/property.*/, "")
            }
            //console.log(url)
            //process.exit()
// Inject “substructure” into the URL if required
            /*
https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/substructure/smiles/C1=NC2=C(N1)C(=O)N=C(N2)N/JSON?MatchTautomers=true&MaxRecords=100         */
...
//        process.exit()
   ...
   //     console.log(url.appendToPubchem(obj.dF))
     //   process.exit()
        /*
https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/substructure/smiles/C1=NC2=C(N1)C(=O)N=C(N2)N/JSON?MatchTautomers=true&MaxRecords=100         */
        request
    ...
    function CmpdSpace (url) {
        ...
        var properties = ["name", "name", "smiles", "cid", "inchi", "inchikey", "substructure"]
        // Slightly changed names of those properties.
        var alias = ["Name", "Cas", "Smiles", "Cid", "Inchi", "InchiKey", "Substructure"];
        // Generates all setters.
        for(var i = 0; i <= properties.length; i += 1) {
            (function (j) {
//     https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/substructure/smiles/C3=NC1=C(C=NC2=C1C=NC=C2)[N]3/XML
                if (alias[j]==="Substructure") {
                    this["set" + alias[j]] = function () {
                        return true
                    }
                } else {
    ...
    function CmpdSpace (url) {
        ...
        var alias = ["Name", "Cas", "Smiles", "Cid", "Inchi", "InchiKey", "Substructure"];
        // Generates all setters.
        for(var i = 0; i <= properties.length -1; i += 1) {
            //     https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/substructure/smiles/C3=NC1=C(C=NC2=C1C=NC=C2)[N]3/XML
            if (alias[i]==="Substructure") {
                this["set" + alias[i]] = function () {
                    this.substructure = true
                    return this
                }
            } else {
