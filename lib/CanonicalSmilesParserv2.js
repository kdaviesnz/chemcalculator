...
// 'CCl' (O)
    ...
    const permanganateOxidation = (alkene_molecule, callback) => {
        ...
        alkene_molecule.CanonicalSMILES.match(/(.*?C[0-9]*?\=)C[0-9]*.*?/g).map(
            (component_SMILES) => {
                MoleculeLookup(db, component_SMILES + "O", "SMILES", true, "permanganateOxidation", lookup_err).then(
                    (mol) => {
                        callback(mol)
                    },
                    (err) => {
...
// CC(C(C1=CC2=C(C=C1)OCO2)O)O isosafrole glycol
 ...
 CC(C)CC1=CC2=C(C=C1)OCO2

 CC(C)C(O)C1=CC2=C(C=C1)OCO2

 OC(C)C(O)C1=CC2=C(C=C1)OCO2 isofrole glycole
 OC(C)C(O)CC1=CC2=C(C=C1)OCO2

 OCC(O)CC1=CC2=C(C=C1)OCO2
 OC(O)CC1=CC2=C(C=C1)OCO2

 ketone:
 [ 'O', 'CC1=CC2=C(C=C1)OCO2', 'C' ]
 OC(C)C(O)C1=CC2=C(C=C1)OCO2 isofrole glycole
 "O" + k[2] + "C(O)" + k[1] .substr(1)
 ...
 */
        ...
        console.log(functionalGroups)
        console.log('Looking up ' + functionalGroups.ketone[1]+ "(=C)" +
...
Looking up CC1=CC2=C(C=C1)OCO2(=C)C
         ...
         */
        MoleculeLookup(db, functionalGroups.ketone[1]+ "(=C)" +
            functionalGroups.ketone[2],
 ...
 */
    ...
    return {
        ...
        replaceCarboxylOxygenWithCR: permanganateOxidationReverse,
