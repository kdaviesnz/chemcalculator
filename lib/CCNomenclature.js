...
and that there is no space between the last substituent and the name of the parent chain.
    ...
    return {
        ...
        calcLongestCarbonChain: () => {
            ...
            chains = molecule_controller.extract_chains( _.map(molecule.atoms, _.clone))
            const longest_chain = chains.reduce(
                (carry, chain) => {
