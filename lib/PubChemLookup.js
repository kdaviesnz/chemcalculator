...
// https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/name/heroin/property/MolecularFormula/JSON
        ...
        request('https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/name/' + name + '/property/' + properties.join(',') + '/JSON', {json: true}, (err, res, body) => {
            if (err) {
                ...
                if (undefined !== body.Fault || undefined !== body.Code) {
                    console.log("There was an error searchByName()  " + name)
                    console.log(body)
                    if (Err) {
                        Err(body)
                    }
    ...
    const searchBySMILES = (SMILES, db, callback, debug) => {
...
// https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/smiles/CCCC/property/MolecularFormula/SDF
        ...
        request('https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/smiles/' + SMILES.replace(/\(\)/g, "") + '/property/' + properties.join(',') + '/JSON', {json: true}, (err, res, body) => {
                if (err) {
                    Err(err);
                   ...
                   */
                    ...
                    if (undefined !== body.Fault || undefined !== body.Code) {
                        if (Err) {
                            console.log("PubchemLookup There was an error searchBySMILES()  " + SMILES)
                            console.log(body.Fault.Message)
                            Err(body)
                            console.log('PubchemLookup searchByName()')
                            process.exit()
                        }
...
// https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/cid/15/property/MolecularFormula,MolecularWeight,CanonicalSMILES/JSON
    ...
    const FetchSubstructuresByListKey = (list_key, db, callback) => {
        ...
        request('https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/listkey/' + list_key + '/property/' + properties.join(',') + '/JSON', { json: true }, (err, res, body) => {
            if (err) {
                ...
                if (undefined !== body.Fault || undefined !== body.Code) {
                    console.log('listkey error - ' + list_key)
                    console.log(body)
                    Err(body)
                    // process.exit()
                } else {
    ...
    const FetchSubstructuresBySMILES = (SMILES, db, callback) => {
        ...
        _log('Calling https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/fastsubstructure/SMILES/' + SMILES + '/cids/JSON?StripHydrogen=true')

        request('https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/fastsubstructure/SMILES/' + SMILES + '/property/CanonicalSmiles,IUPACName/JSON?StripHydrogen=true', { json: true }, (err, res, body) => {
            if (err) {
                Err(err)
