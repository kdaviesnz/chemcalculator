        ...
        case "carboxylic acid":
            end_product_JSON_object.functional_groups.map(
                (functional_group)=> {
                    switch (functional_group) {
                        case "ketone":
                            if ((rule.catalyst === "pyridine" || rule.catalyst === "sodium acetate") && rule.reagents.indexOf("acyclic carboxylic anhydride")){
                                CanonicalSMILESParserV2(end_product_JSON_object, db, rule, child_reaction_as_string, render, Err).carboxylicAcidToKetoneReverse()
                            }
                            break
                    }
                    ...
                    switch (functional_group) {
                        ...
                        case "imide":
                            if (rule.reagents.indexOf("strong acid")  > -1) {
                                CanonicalSMILESParserV2(end_product_JSON_object, db, rule, child_reaction_as_string, render, Err).pinacolRearrangement()
                            }
                            break
                    }
                }
                    ...
                    switch (functional_group) {
                        case "methyl ketone":
                            if (
                                ...
                                CanonicalSMILESParserV2(end_product_JSON_object, db, rule, child_reaction_as_string, render, Err).wackerOxidation()
                            }
                            break
                    }
                }
